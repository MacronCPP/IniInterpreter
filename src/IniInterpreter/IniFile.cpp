#include <IniInterpreter/IniFile.h>
...
namespace IniInterpreter
{
    ...
    {
        if(sections.count(name) == 0)return nullptr;
    ...
    std::map<std::string, Section>::iterator IniFile::begin()
    {
        return sections.begin();
    }

    std::map<std::string, Section>::iterator IniFile::end()
    {
        return sections.end();
    }
        ...
        do
        {
            ...
            switch(commandType)
            {
                case NEWSECTION:
                    ...
                    writeMap = &(sections[name].Variables);
                    break;
